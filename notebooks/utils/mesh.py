...
class Mesh:
    """
    Class that contains the mesh where fractal tree is grown. It must be Wavefront .obj file.
    Be careful on how the normals are defined. It can change where an specified angle will go.
    ...
    Attributes:
        ...
        connectivity (array): a numpy array that contains all the connectivity
        of the triangles of the mesh. connectivity[i,j], where i is the triangle
        index and j=[0,1,2] is node index.

        normals (array): a numpy array that contains all the normals of the triangles of the mesh.
        normals[i,j], where i is the triangle index and j=[0,1,2] is normal coordinate (x,y,z).

        node_to_tri (dict): a dictionary that relates a node to the triangles that it is connected.
        It is the inverse relation of connectivity. The triangles are stored as a list for each node.
    ...
    def __init__(self, filename=None, verts=None, connectivity=None):
        ...
        for i in range(len(self.connectivity)):
            ...
            u = self.verts[self.connectivity[i, 1], :] - \
                self.verts[self.connectivity[i, 0], :]
            v = self.verts[self.connectivity[i, 2], :] - \
                self.verts[self.connectivity[i, 0], :]
            n = np.cross(u, v)
            self.normals[i, :] = n/np.linalg.norm(n)

        self.tree = cKDTree(verts)
        self.centroids = (self.verts[self.connectivity[:, 0], :] +
                          self.verts[self.connectivity[:, 1], :] + self.verts[self.connectivity[:, 2], :])/3.
  ...
  #                      print w
    ...
    def writeOBJ(self, path):
        """ writes points and faces data as .obj file """
    ...
    def project_point_check(self, point, node):
        ...
        pre_projected_point = point-vertex_normal * \
            np.dot(point-self.verts[node], vertex_normal)
        # Calculate the distance from point to plane (Closest point projection)
        CPP = []
        for tri in triangles:
            CPP.append(np.dot(pre_projected_point -
                       self.verts[self.connectivity[tri, 0], :], self.normals[tri, :]))
        CPP = np.array(CPP)
     #   print 'CPP=',CPP
    ...
    def writeVTU(self, filename, verts=None, connectivity=None, scalars=None, vectors=None):
        if not verts and not connectivity:
            verts = self.verts
    ...
    def Bmatrix(self, element):
        ...
        e1 = (nodeCoords[1, :] - nodeCoords[0, :]) / \
            np.linalg.norm(nodeCoords[1, :] - nodeCoords[0, :])
        e2 = ((nodeCoords[2, :] - nodeCoords[0, :]) -
              np.dot((nodeCoords[2, :] - nodeCoords[0, :]), e1)*e1)
    ...
    def computeGeodesic(self, nodes, nodeVals, filename=None):
...
#        M = sp.lil_matrix((nNodes, nNodes))
        ...
        AT = np.linalg.solve(
            K[iActive, jActive], F[activeNodes, 0]-np.dot(K[iKnown, jKnown], nodeVals))
        ...
        if filename is not None:
            self.writeVTU(filename, self.verts,
                          self.connectivity, ATglobal, Xs)
    ...
    def computeLaplace(self, nodes, nodeVals, filename=None):
        ...
        if filename is not None:
            self.writeVTU(filename, self.verts,
                          self.connectivity, Tglobal, None)
